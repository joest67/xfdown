...
class XF:
    ...
    def online_v(self, lists):
        ...
        cmd = ['wget', '-c', '-O', filename, '--header', 'Cookie:FTN5K=%s' %
               self.filecom[num], self.filehttp[num]]
    ...
    def __init__(self):
        ...
        opener.addheaders = [
            ('User-Agent', 'Mozilla/5.0'), ("Referer", "http://lixian.qq.com/main.html")]
    ...
    def __request(self, url, data=None, savecookie=False):
        ...
        if savecookie:
            if hasattr(self, "pswd"):
                self.cookieJar.save(ignore_discard=True, ignore_expires=True, userinfo="%s#%s" % (
                    self.__qq, self.hashpasswd))
            else:
    ...
    def __getverifycode(self):
        ...
        urlv = 'http://check.ptlogin2.qq.com/check?uin=%s&appid=567008010&r=%s' % (
            self.__qq, random.Random().random())

        str = self.__request(url=urlv)
        verify = eval(str.split("(")[1].split(")")[0])
        verify = list(verify)
        if verify[0] == '1':
            imgurl = "http://captcha.qq.com/getimage?aid=567008010&r=%s&uin=%s" % (
                random.Random().random(), self.__qq)
    ...
    def __request_login(self):
        ...
        urlv = "http://ptlogin2.qq.com/login?u=%s&p=%s&verifycode=%s" % (self.__qq, self.passwd, self.__verifycode[
                                                                         1]) + "&aid=567008010&u1=http%3A%2F%2Flixian.qq.com%2Fmain.html&h=1&ptredirect=1&ptlang=2052&from_ui=1&dumy=&fp=loginerroralert&action=2-10-&mibao_css=&t=1&g=1"
        str = self.__request(url=urlv)
    ...
    def __getlogin(self):
        self.__request(url="http://lixian.qq.com/handler/log_handler.php",
                       data={'cmd': 'stat'}, savecookie=True)
        ...
        f.close()
        str = self.__request(
            url=urlv, data={"g_tk": get_gtk(skey)}, savecookie=True)
        return str
        ...
        if res["msg"] == _('未登录!'):
            ...
            for num in range(len(res['data'])):
                ...
                if size == 0:
                    ...
                    percent = str(
                        index['comp_size'] / size * 100).split(".")[0]
                ...
                size = "%.1f%s" % (size, _dw)
                item = (
                    size, percent, _(self.filename[num]), status, tasktype, fileurl)
                result.append(item)
        return result
    ...
    def gethttp(self, filelist):
        ...
        for num in filelist:
            data = {'hash': self.filehash[
                num], 'filename': self.filename[num], 'browser': 'other'}
            str = self.__request(urlv, data)
            self.filehttp[num] = (
                re.search(r'\"com_url\":\"(.+?)\"\,\"', str).group(1))
            self.filecom[num] = (
                re.search(r'\"com_cookie":\"(.+?)\"\,\"', str).group(1))
